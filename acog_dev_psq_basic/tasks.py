...
def process_infile(self,path_: str, out_path: str)->None:
    ...
    table_name=path_.replace('/','_').replace('-','_')+out_path.replace('/', '_').replace('-', '_')
    print(table_name)
    if not sql.check_for_table(table_name):
        sql.create_table(table_name, 'djedi')
        print('created table')
        infiles=os.listdir(path_)
        out_files=[x+'_tf' for x in infiles]
        print(infiles)
        if len(infiles)==0:
            print('The input DIR here is empty')
        
        non_processed_files=[(infile,outfile) for infile,outfile in zip(infiles,out_files) if sql.check_if_both_infile_outfile_in_table(table_name,infile,outfile)==False]
        ...
        for i, o in non_processed_files:
            try:
                ...
                df=pd.read_csv(os.path.join(path_,i)).to_html(os.path.join(out_path,o.replace('.csv_tf', '.html')))
                sql.update_table(table_name,{'infile':i,'outfile':o,'status':1})
